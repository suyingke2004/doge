...
class TestLongTermMemoryToolInvocation(unittest.TestCase):
    ...
    def test_system_prompt_for_long_term_memory(self):
        """测试系统提示中是否包含长期记忆相关的指导"""
        # 通过mock LLM配置来避免API密钥问题
        with patch.object(DogAgent, '_configure_llm') as mock_configure_llm:
            # 创建一个mock的LLM对象
            mock_llm = MagicMock()
            ...
            prompt_template = dog_agent.agent_executor.agent.runnable.first.mapper['input'].messages[0].prompt.template
            self.assertIn("如果了解到用户的个人信息", prompt_template)
            self.assertIn("可以使用update_long_term_memory工具", prompt_template)
